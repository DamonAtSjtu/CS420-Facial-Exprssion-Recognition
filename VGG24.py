...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        conTensor1 = net1.features(inputs)
        conTensor2 = layers(inputs,net2).view(bs,-1)
        print(conTensor1.shape,conTensor2.shape).view(bs,-1)
...
def PublicTest(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(PublicTestloader):
        ...
        input1 = perturb(input1,label,0.01)
        input1 = torch.Tensor(input1)
        input1 = np.reshape(input1, newshape = [spl,10,3,44,44])
        input2 = np.reshape(input2,newshape = [10*(bs-spl),3,44,44])
        label = []
        for i in range(spl,bs):
            for j in range(10):
                label.append(targets[i].item())
        
        label = torch.LongTensor(label)
        input2 = perturb(input2,label,0.01)
        input2 = torch.Tensor(input2)
        input2 = np.reshape(input2, newshape = [(bs-spl),10,3,44,44])
        inputs = np.vstack((input1,input2))
        '''
        inputs = torch.Tensor(inputs)
        ...
        utils.progress_bar(batch_idx, len(PublicTestloader), 'Loss: %.3f | Acc: %.3f%% (%d/%d)'
                           % (PublicTest_loss / (batch_idx + 1), float(100.*correct)/float(total), correct, total))
        

    # Save checkpoint.
    PublicTest_acc = float(100.*correct)/float(total)
    print("acc:",PublicTest_acc)
    print("loss:",PublicTest_loss)
...
def PrivateTest(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(PrivateTestloader):
        ...
        utils.progress_bar(batch_idx, len(PrivateTestloader), 'Loss: %.3f | Acc: %.3f%% (%d/%d)'
                           % (PrivateTest_loss / (batch_idx + 1),float(100. * correct) / float(total), correct, total))
    # Save checkpoint.
    PrivateTest_acc = float(100.*correct)/float(total)
    print("acc:",PrivateTest_acc)
    print("loss:",PrivateTest_loss)


net.load_state_dict(torch.load('./forTEST/1/PublicTest_ad251_model.t7',map_location='cpu')['net'])
